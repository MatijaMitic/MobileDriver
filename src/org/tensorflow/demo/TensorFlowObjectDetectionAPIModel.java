    ...
    for (int i = 0; i < outputScores.length; ++i) {
      final RectF detection =
        ...
        if(label.equals("car") || label.equals("bus") || label.equals("truck") || label.equals("person") ) {
            ...
            double apx_distance = Math.round(Math.pow((1-(outputLocations[4 * i + 3] - outputLocations[4 * i + 1])),4)*10)/10;
            boolean isDangerous = (apx_distance <= 0.5 && mid_x > 0.3 && mid_x < 0.7);
            pq.add(
                    new Recognition("" + i, label , outputScores[i], detection, isDangerous));
        }
